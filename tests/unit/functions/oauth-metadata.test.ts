...
describe('oauth metadata helpers', () => {
  describe('getRequestOrigin', () => {
    it('strips path and query', () => {
      expect(
        getRequestOrigin(
          'https://cabuya.org/.well-known/oauth-protected-resource'
        )
      ).toBe('https://cabuya.org');
    });
  });

  describe('buildOAuthProtectedResourceMetadata', () => {
    it('matches request origin for apex and v3', () => {
      const apex = buildOAuthProtectedResourceMetadata('https://cabuya.org');
      expect(apex.resource).toBe('https://cabuya.org');
      expect(apex.authorization_servers).toEqual(['https://cabuya.org']);
      expect(apex.bearer_methods_supported).toContain('header');
      expect(apex.scopes_supported).toContain('public:read');

      const v3 = buildOAuthProtectedResourceMetadata('https://cabuya.org');
      expect(v3.resource).toBe('https://cabuya.org');
      expect(v3.authorization_servers).toEqual(['https://cabuya.org']);
    });
  });
