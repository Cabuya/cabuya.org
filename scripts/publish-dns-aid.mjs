 ...
 *   CF_ZONE_ID     — zone id for cabuya.org
 *                    (or set CF_ZONE_NAME=cabuya.org to look it up)
 *
 * Optional:
...
const ZONE_NAME = process.env.CF_ZONE_NAME || 'cabuya.org';
...
async function main() {
  ...
  console.log(
    '\nRe-scan: POST https://isitagentready.com/api/scan {"url":"https://cabuya.org"}'
  );
}
